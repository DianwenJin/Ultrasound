...
image(env_gray);
colormap(gray(128));
xlswrite('C:\Users\Classic\Documents\MATLAB\testCircle2.xlsx',[env_gray]);
...
r=6/1000; % Radius of cyst [m]
xc=0/1000; % Place of cyst [m]
% zc= 15/1000+z_start;
zc= 2*z_start;
inside = ( ((x-xc).^2 + (z-zc).^2) < r^2); %判断，内部则返回1
amp = amp .* (1-inside);

% 建立内部0.3回声
% amp = amp.*(1-inside) + 0.3*amp.*inside;
% amp = amp .* (1-inside);    amp=0,囊肿特征，尝试其他amp看看差异
